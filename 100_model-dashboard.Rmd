...
```{r}
growthResG <- reactive({ as_tibble(growthModel(dlTrend = input$dlTrend, dkTrend = input$dkTrend, dMFP = input$dMFP))  
})
...
growthRes <- reactive({ 
  withProgress(message = "Calculating GDP", value = 0, {
    incProgress()
    as_tibble(growthModel(dlTrend = input$dlTrend, dkTrend = input$dkTrend, dMFP = input$dMFP))  
  })
})
#coeff <- sqrt(sum(growthRes$GDP^2))
