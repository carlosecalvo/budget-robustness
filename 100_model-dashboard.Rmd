...
testMFP <- rep(NA, 76)
testMFP[1] <- 0.0112
baseGrowth <- growthModel(dMFP = testMFP, dlTrend = 0.00837, dkTrend = 0.0181) %>% 
  as_tibble()
...
baseDebt <- debtModel(intRate = intRate, intRateTrend = 0.057630, intRateSet = 1, GDP = baseGDP) %>% 
  as.tibble()
...
```{r}
growthResG <- reactive({ as_tibble(growthModel(dlTrend = input$dlTrendG, dkTrend = input$dkTrendG))  
})
#coeff <- sqrt(sum(growthRes$GDP^2))

renderPlot({ 
  gdp_pg <- ggplot(data = growthResG(), aes(x = 1:76))+
  geom_line(aes(y = GDP), size = 1.1)+
  geom_line(data = baseGrowth, aes(y = GDP), color = "red", alpha = 0.5)+
  # geom_line(aes(y = MFP*coeff)) +
   scale_y_continuous(name = "GDP", labels = scales::label_dollar(), breaks = scales::breaks_extended(n=4))+
    xlab("Time")+
    ggtitle("GDP over time", subtitle = "Red line = Baseline")
  #                    sec.axis = sec_axis(~./coeff, name =  "MFP"))
  #plotly::ggplotly(p, tooltip = c('y', 'x'))
  
  mfp_pg <- ggplot(data = growthRes(), aes(x = 1:76))+
  geom_line(aes(y = MFP), size = 1.1)+
  geom_line(data = baseGrowth, aes(y = MFP), color = "red", alpha = 0.5)+
  # geom_line(aes(y = MFP*coeff)) +
...
debtResultG <- reactive({ 
  ...
  as_tibble(debtModel(intRate = intRate, intRateTrend = input$intRateTrendD, intRateSet = input$intRateSetD, GDP = GDPInput()))
  })

renderPlot({
  debt_p <- ggplot(data = debtResultG(), aes(x = 1:76))+
  geom_line(aes(y = Debt), size = 1.1)+
  geom_line(data = baseDebt, aes(y = Debt), color = "red", alpha = 0.5)+
  # geom_line(aes(y = MFP*coeff)) +
...
Integrated Growth and Debt Models
=======================================================================
...
growthRes <- reactive({ 
  withProgress(message = "Calculating GDP", value = 0, {
    incProgress()
    as_tibble(growthModel(dlTrend = input$dlTrend, dkTrend = input$dkTrend))  
  })
})
#coeff <- sqrt(sum(growthRes$GDP^2))
